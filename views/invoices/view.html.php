	...
	function detail($tpl = null) {
		...
		foreach($model->getCharges() as $charge) {
			if($campers[$charge->familyid]["charges"] == null) {
				...
				$campers[$charge->familyid]["total"] = (float)preg_replace("/,/", "",  $charge->amount);
			} else {
		...
		foreach($model->getCredits() as $credit) {
			...
			$campers[$chargeid]["total"] += (float)$credit->amount;
		}
		...
		header("Content-Disposition: attachment; filename=invoicedata.csv;");
		...
		foreach ($campers as $camperid=>$camper) {
			...
			for($i=1; $i<7; $i++) {
				if(count($camper["children"]) > 0) {
					...
					echo preg_replace("/&#039;/", "'", preg_replace("/,/", "", $child->age));
				} else {
					echo ",,,,,,";
				}
			}
			...
			echo "," . sprintf("%01.2f", $camper["total"]); //, 2, ".", "");
			for($i=1; $i<10; $i++) {
